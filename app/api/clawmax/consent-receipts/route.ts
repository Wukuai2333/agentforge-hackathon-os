...
export async function DELETE(request: Request) {
  ...
  if (existing.status !== "revoked") {
    await runtime.DB.batch([
      runtime.DB.prepare("UPDATE clawmax_consent_receipts SET status='revoked',revoked_at=?,updated_at=? WHERE receipt_id=? AND status!='revoked'").bind(now, now, receiptId),
  ...
  return Response.json({ receiptId, status: "revoked", purgeStatus: "pending" }, { status: 202 });
}
