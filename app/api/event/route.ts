...
export async function GET(request: Request) {
  ...
  const config = await runtime.DB.prepare(`SELECT event_name AS eventName, starts_at AS startsAt, ends_at AS endsAt,
    timezone, discord_url AS discordUrl, registration_open AS registrationOpen, updated_at AS updatedAt
...
export async function PUT(request: Request) {
  ...
  const input = await request.json() as { eventName?: string; startsAt?: number | null; endsAt?: number | null; timezone?: string; discordUrl?: string; registrationOpen?: boolean };
  ...
  await runtime.DB.prepare(`INSERT INTO event_configuration (id,event_name,starts_at,ends_at,timezone,discord_url,registration_open,updated_at)
    VALUES ('primary',?,?,?,?,?,?,?) ON CONFLICT(id) DO UPDATE SET event_name=excluded.event_name,starts_at=excluded.starts_at,
    ends_at=excluded.ends_at,timezone=excluded.timezone,discord_url=excluded.discord_url,registration_open=excluded.registration_open,updated_at=excluded.updated_at`)
    .bind(input.eventName?.trim().slice(0, 120) || "Personal Agent Hackathon", startsAt, endsAt, input.timezone?.trim().slice(0, 80) || "America/New_York", discordUrl, input.registrationOpen === false ? 0 : 1, updatedAt).run();
